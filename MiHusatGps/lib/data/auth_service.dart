    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (token.isNotEmpty) {
          ...
          // Guardar contraseña para validación de comandos
          await StorageService.savePassword(clave);
        }
        
        // Obtener rolId del usuario desde la API después del login
        int? rolId;
        try {
          final userProfile = await UserService.obtenerUsuario(userId);
          if (userProfile != null && userProfile.rolId != null) {
            rolId = userProfile.rolId;
            await StorageService.saveUserRolId(rolId!);
            debugPrint('✅ RolId obtenido después del login: $rolId');
          } else {
            debugPrint('⚠️ No se pudo obtener rolId del usuario después del login');
          }
        } catch (e) {
          debugPrint('⚠️ Error al obtener rolId después del login: $e');
          // Continuar sin rolId, no es crítico para el login
        }
